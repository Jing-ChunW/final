...
void doeverything()
{
   ...
   while (ping_num < 4) {
      if (ping_num == 0) {
         ...
         while (end == 0 && i < 256 && count_new < 100 && ping_block > 22) {
            if(uart.readable()){
               char recv[1];
         ...
         if (haveread2 == 1) {
            ...
            if (double_check >= 1) {
               ...
               if (goal_angle > 0 && goal_angle < 180) {
                  ...
                  steps1 = 0;
                  car.turn(-20, -0.1);
                  while (steps1*6.5*3.14/32 < 10.5*2*3.14*103/360) {
                     ThisThread::sleep_for(100ms);
                  }
         ...
         steps1 = 0;
         car.turn(20, -0.1);
         while (steps1*6.5*3.14/32 < 11*2*3.14*80/360) {
            ThisThread:: sleep_for(100ms);
         }
         steps1 = 0;
         steps0 = 0;
...
void Line_mode(){
   ...
   while(1){
      ...
      count_new = 0;
      int nocount = 0;
      while (end == 0 && i < 256 && count_new < 100 && ping_block > 15) {
         if(uart.readable()){
            char recv[1];
            uart.read(recv, sizeof(recv));
            y = 1;
